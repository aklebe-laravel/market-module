...
class DatabaseSeeder extends BaseModelSeeder
{
        ...
        foreach ($userIds as $userId) {
            ...
            if ($mediaItems->count()) {
                ...
                Product::where('user_id', $userId)->each(function ($product) use ($mediaItems) {
                    $imagesPerProduct = rand(config('seeders.users.media_items.count_min_product_images_per_product', 2), config('seeders.users.media_items.count_max_product_images_per_product', 2));
                    // syncWithoutDetaching() better than attach() to avoid errors if items already exists
                    $product->mediaItems()->syncWithoutDetaching($mediaItems->random($imagesPerProduct)->pluck('id')->toArray());
                });
            }
            ...
            if ($mediaItems->count()) {
                ...
                $user->mediaItems()->syncWithoutDetaching($mediaItems->random(rand(1, ($mediaItems->count() >= 5) ? 5 : $mediaItems->count()))->pluck('id')->toArray());
            }
    ...
    private function AssignCategoryRelations(string $timestamp): void
    {
        ...
        $mediaItems = MediaItem::where('created_at', '>=', $timestamp)
        ...
        Category::where('created_at', '>=', $timestamp)
                ->each(function ($category) use (
                    ...
                    if ($minProductsPerCategory) {
                        // syncWithoutDetaching() better than attach() to avoid errors if items already exists
                        $category->products()
                                 ->syncWithoutDetaching($products->random(rand($minProductsPerCategory, $maxProductsPerCategory))
                                                                 ->select('id')
                                                                 ->pluck('id')
                                                                 ->toArray());
                    }
                    ...
                    if ($minMediaItemsPerCategory) {
                        // syncWithoutDetaching() better than attach() to avoid errors if items already exists
                        $category->mediaItems()
                                 ->syncWithoutDetaching($mediaItems->random(rand($minMediaItemsPerCategory, $maxMediaItemsPerCategory))
                                                                   ->pluck('id')
                                                                   ->toArray());
                    }
                });
    ...
    private function AggregateRatings(string $timestamp): void
    {
        ...
        // -------------------------------------------------
        Product::where('created_at', '>=', $timestamp)
